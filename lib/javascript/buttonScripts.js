...
function ItemForm(row) {
	if (row === 0) {
		...
		var tr = document.getElementById('row' + row).children;
		var isbn = tr[0].innerHTML;
		var author = tr[1].innerHTML;
		var title = tr[2].innerHTML;
		var shelf = tr[3].innerHTML;
		var item = tr[4].innerHTML;
		var button = "Edit Item";
	}

	var htmlCode = `<input type="hidden" name="row" id="row" value="${row}"
		<label for="title">ISBN:</label>
		<input type="text" name="isbn" id="isbn" maxlength="13" value="${isbn}" required />
		<label for="title">Author:</label>
		<input type="text" name="author" id="author" maxlength="50" value="${author}" required />
		<label for="title">Title:</label>
		<input type="text" name="title" id="title" maxlength="100" value="${title}" required />
		<label for="title">Shelf Location:</label>
		<input type="text" name="shelf" id="shelf" maxlength="50" value="${shelf}" required />
		<label for="title">Item Location:</label>
		<input type="number" name="item" id="item" min="0" max="255" value="${item}" required />
		<input type="submit" id="add" value="${button}" />`;
